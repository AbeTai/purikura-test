...
- `fast`
  ...
  - Face Landmarkerは移動中は毎フレーム、静止中は1フレーム再利用する。
  - Segmenterは静止時最大16フレーム、移動量に応じて2フレームまたは毎フレーム更新する。
...
- Fast previewでは顔ランドマークを移動中は毎フレーム、静止中は1フレームだけ再利用している。
- Fast previewではmotionに応じてエフェクト強度を減衰する。
- SelfieMulticlass Segmenterは固定間引きだけでなく、顔移動量と `mask_age_ms` を見て前倒し更新する。静止時はFastで最大16フレーム/約1秒再利用し、中程度のmotionでは2フレームごと、大きなmotionでは毎フレーム更新する。移動中のmask age上限は150ms。
- 静止時のFace Landmarkerは1フレームだけ結果を再利用し、移動検出後は毎フレーム更新する。
- 複数人の部位maskは部位ごとにまとめてblurし、小さいmaskのblendは有効ROIだけで計算する。
- Fast版の旧メイク処理とドール版で重複していたチーク、リップ、キャッチライト処理はドール版へ一本化した。
...
- `processing_ms <= 58` かつ `frame_age_ms <= 100` が1秒以上続いたら一段ずつ640pxへ戻す。
...
- 部位maskのblur集約、sparse maskのROI blend、Fastメイクの重複削除を実装済み。
- 静止時のFace Landmarkerを1フレーム再利用し、Segmenterを最大16フレーム再利用する適応更新を実装済み。
- 参考画像の2人顔を1280x720入力、内部448pxで測り、Fast平均58.4ms、17.1fpsを確認した。

...
- 参考画像寄せの「ドール盛り」詳細補正: 陶器肌、丸い目、アイライン、まつ毛、涙袋、瞳グロス、頬グラデーション、リップグロス、髪なめらか化、白背景化、ソフトグロー
- 透過PNGフレームのアップロードと合成
- 撮影画像と加工設定のSQLite保存
...
`Doll Details` セクションには、参考画像のような白くふわっとしたプリクラ感を作るための詳細スライダーがあります。古典CV中心の実装で、追加のGAN/ONNXモデルは使っていません。

- `Doll intensity`: ドール盛り全体のかかり具合です。下記の詳細補正の共通倍率として働きます。
- `Porcelain skin`: 肌のLチャンネルを均し、影や色ムラを抑えてアイボリー/ピンク白の陶器肌へ寄せます。目、眉、唇は保護します。
- `Eye roundness`: 目の周辺を縦方向に強めて局所変形し、丸い大きな目に寄せます。
- `Eye liner`: 上下まぶたのランドマーク曲線に沿って半透明の濃色ラインを重ねます。
- `Lash emphasis`: 上まぶた外側へ短いストロークを足し、まつ毛感を出します。
- `Lower eyelid`: 下まぶた下に明るい涙袋ハイライトと薄い影ラインを入れます。
- `Iris gloss`: 虹彩周辺のリングと複数の白いキャッチライトで瞳のツヤを足します。
- `Cheek gradient`: 頬に大きめのピンクグラデーションを重ねます。
- `Lip gloss`: 唇の外側マスクに赤ピンクを乗せ、中央に細いハイライトを入れてぷるっとした質感にします。
- `Hair silk`: hair mask内を低強度で平滑化し、暗部を少し持ち上げて髪をなめらかに見せます。
- `Background high key`: background maskだけを白/薄ピンクへ寄せ、白いスタジオ背景のようにします。
- `Soft glow`: 明部から低解像度のbloomを作り、全体を薄くscreen合成してプリクラ特有のふわっとした白さを作ります。

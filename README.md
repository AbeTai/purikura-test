...
- Fast版の負荷に応じた内部処理幅 `640 / 512 / 448px` の自動調整
- デバッグオーバーレイによるランドマーク、マスク、顔パーツ領域の確認
- `/api/performance` とUIのPerformanceパネルによる直近の処理時間、FPS、フレーム破棄数、表示/解析/mask年齢の確認
...
- 処理が80ms超または表示フレーム年齢が120ms超の状態が続くと、内部処理幅を `640 → 512 → 448px` と段階的に下げます。処理が1秒以上安定すると一段ずつ640pxへ戻します。
- 処理幅を変更した直後はセグメンテーションmaskを破棄して再取得し、異なる縮尺のmaskが混ざらないようにします。
- フレームPNG合成は最終解像度で行うため、アップロードしたフレームの表示サイズは維持されます。
- Face Landmarkerは移動中は毎フレーム、静止中は1フレームだけ結果を再利用します。静止中でも最大約1フレーム遅れで再検出し、移動を検出した後は毎フレーム更新へ戻します。
- SelfieMulticlass Segmenterは静止時は最大16フレーム、約1秒までmaskを再利用します。顔移動が中程度なら2フレームごと、大きい場合は毎フレーム再実行し、移動中のmask age上限は約150msです。
- 顔が小さく動いたときは前回マスクを現在の顔中心へ平行移動して追従させ、大きく動いたときは古いマスクを混ぜません。
- 肌、頬、額、顎、鼻などの補正は、全画面ではなく `head_roi` に限定します。
- bilateral filterとblurは共有の1回にまとめ、頬/額/顎ごとに重いフィルタを再実行しません。
- 目、唇、鼻筋などの小さいmask合成は有効領域だけを計算し、全画面のfloat変換を避けます。
- 複数人の部位maskは、全員分を描画してから部位種別ごとに1回だけblurします。
- Fast版では旧チーク/リップ/キャッチライトとドール版の重複処理を避け、ドール版へ一本化します。
...
- 急に顔が大きく動いた瞬間は、残像を抑えるために一時的に補正が薄くなることがあります。
...
| --- | --- | --- |
| 優先するもの | 補正品質 | 低遅延、FPS |
| 内部処理解像度 | 入力解像度中心 | 横幅640 / 512 / 448pxを負荷で自動選択 |
| 顔検出 | 毎フレーム | 移動中は毎フレーム、静止中は1フレーム再利用 |
| セグメンテーション | 4フレームごと、mask ageが古い場合は前倒し | 静止時は最大16フレーム、移動時は2または1フレームごと |
| 補正範囲 | 広め、部位別に丁寧 | head ROI中心 |
| 重いフィルタ | 肌/部位ごとに適用 | 共有して回数削減 |
| おすすめ用途 | 品質確認、撮影結果重視 | ライブプレビュー、実運用 |

起動時の初期設定は `Fast` です。基本運用では、プレビュー中は `Fast`、補正の見た目を詰めるときは `Quality` を使う想定です。撮影保存時は、UIで `Fast` を選んでいても最新のrawフレームを `Quality` として再処理してDBへ保存します。これにより、ライブ表示の追従性と保存画像の盛れ具合を分離しています。

Mac内蔵カメラはiPhone/Continuity Cameraより取得間隔や初期化直後の挙動が不安定になりやすいため、カメラreadは約30fpsに抑え、プレビューpublish判定はフレーム番号だけでなく経過時間も使います。加工が少し遅れても更新を全部捨てず、静止画のまま固まらないようにしています。
...
現在の初期値は、丸く大きい目と虹彩、少し短い顎、均一な白肌、艶のある暗髪、鮮やかな赤ピンク唇を優先しています。チークと全体グローは控えめにし、顔の輪郭と髪の質感が白くぼやけすぎない配分です。
...
Fast版の内部処理幅を固定して比較する場合:

```bash
uv run python scripts/benchmark_pipeline.py --profile fast --fast-process-width 448 --image /path/to/image.png
```
...
この値は実行環境、入力画像、顔の数、顔の大きさ、Debug overlayの有無で変わります。参考画像の2人顔を1280x720入力、内部448pxで測った結果は平均58.4ms、17.1fpsでした。Fast版の目標は、Mac内蔵カメラ相当の入力で平均15fps以上です。

## APIメモ

- `GET /api/cameras`: 利用可能なカメラ候補を返します。現在使用中のカメラはmacOS/OpenCVで再Openしないよう、probeせずavailableとして返します。
- `GET /api/effects`: 現在の補正設定を返します。
- `PUT /api/effects`: `processing_profile` を含む補正設定を更新します。
- `GET /api/performance`: 直近の `processing_ms`, `encode_ms`, `effective_fps`, `dropped_frames`, `discarded_processed_frames`, `frame_age_ms`, `landmark_age_ms`, `mask_age_ms`, `motion_factor`, `publish_interval_ms`, `publish_lag_frames`, `preview_stall_ms`, `published_frame_id`, `latest_raw_frame_id`, `process_width`, `profile` を返します。
- `GET /api/preview.mjpeg`: 加工済みライブプレビューをMJPEGで返します。
- `POST /api/captures`: 最新のrawカメラフレームを撮影用に固定し、`Quality` profileで再処理したJPEGをDBへ保存します。

`processing_profile` は `"quality"` または `"fast"` を指定します。ライブプレビューではこの値をそのまま使います。撮影保存では `Fast` 表示中でも `processing_profile` を `"quality"` にした撮影設定を `effect_settings_json` に保存するため、保存画像が撮影用Quality再処理だったことを追跡できます。

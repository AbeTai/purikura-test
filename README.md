...
- `GET /api/performance`: 直近の `processing_ms`, `encode_ms`, `effective_fps`, `dropped_frames`, `discarded_processed_frames`, `frame_age_ms`, `landmark_age_ms`, `mask_age_ms`, `motion_factor`, `publish_interval_ms`, `publish_lag_frames`, `preview_stall_ms`, `published_frame_id`, `latest_raw_frame_id`, `profile` を返します。

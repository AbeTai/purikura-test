...
- Face Landmarkerは毎フレーム実行し、顔移動量を測って補正強度を動的に調整します。
- SelfieMulticlass Segmenterは静止時は最大8フレームまで再利用し、顔移動が中程度以上、またはmask ageが約150msを超える場合は前倒しで再実行します。
...
| 顔検出 | 毎フレーム | 毎フレーム |
| セグメンテーション | 4フレームごと、mask ageが古い場合は前倒し | 静止時は最大8フレーム再利用、移動時やmask age超過時は前倒し |

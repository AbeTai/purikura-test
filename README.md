...
- `/api/performance` による直近の処理時間、FPS、フレーム破棄数、表示フレーム年齢の確認
...
- `GET /api/performance`: 直近の `processing_ms`, `encode_ms`, `effective_fps`, `dropped_frames`, `discarded_processed_frames`, `frame_age_ms`, `motion_factor`, `published_frame_id`, `latest_raw_frame_id`, `profile` を返します。
